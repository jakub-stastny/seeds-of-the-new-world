...
#+begin_example "Can the US really default?"
  Not in the usual way. Most countries eventually go broke — they can't pay their debts. The US is different: it prints the world’s reserve currency, so it can always hit ‘print’ and pay[fn::The US Treasury borrows money, but the Federal Reserve — technically independent — can create dollars to buy that debt if markets stop doing so.]. But here’s the catch: printing money doesn’t make the problem go away — it just shifts it[fn::This is not a default in name, but it is in effect: if you’re paid back in money that buys much less, you’ve still lost.]. The more they print, the less that money is worth. At some point, people stop trusting the dollar. Then they dump it. And when trust dies, the dollar dies — and with it, the US economy as we know it[fn::Once global trust in the dollar breaks, trade contracts, foreign reserves, and energy deals start shifting elsewhere — which has already begun.].
#+end_example
